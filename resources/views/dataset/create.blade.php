            ...
            <form id="form-upload-dataset" action="{{ route('dataset.store') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                                        ...
                                        @foreach ($characteristics as $characteristic)
                                            <div class="form-check d-flex align-items-center">
                                                ...
                                                <input name="characteristics[]"
                                                    class="form-check-input ms-auto characteristic" type="checkbox"
                                    ...
                                    <div class="card-body" id="associatedTasks">
                                        @foreach ($associatedTasks as $associatedTask)
                                            <div class="form-check d-flex align-items-center">
                                                ...
                                                <input name="associatedTasks[]"
                                                    class="form-check-input ms-auto associatedTasks" type="checkbox"
                                        ...
                                        @foreach ($featureTypes as $featureType)
                                            <div class="form-check d-flex align-items-center">
                                                ...
                                                <input name="featureTypes[]" class="form-check-input ms-auto featureTypes"
                                                    type="checkbox" value="{{ $featureType->id }}"
                                                    style="border-color: #38527E;">
                                            </div>
                                        @endforeach
                                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <button id="simpan" type="submit" id="btnNext"
                                    class="btn fs-6 text-light mt-4 px-4 flex float-end"
...
@section('scripts')
    <script>
        ...
        fileInput.addEventListener('input', function() {
            // Konversi FileList ke array baru
            selectedFiles = Array.from(fileInput.files);
            updateFileList();
        });

        // Fungsi untuk memperbarui daftar file di tampilan
        function updateFileList() {
            listFiles.innerHTML = "";
            selectedFiles.forEach((file, index) => {
                let span = document.createElement("span");
                span.innerHTML = `${file.name} <a onclick="removeFile(${index})"><i class="fas fa-times text-danger"></i></a>
        ${index < selectedFiles.length - 1 ? ' | ' : '' }`;
                listFiles.appendChild(span);
            });
            updateInputFile();
